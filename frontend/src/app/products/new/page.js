...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append actual File objects from the images state
    images.forEach((img) => {
      if (img.file) {
    ...
    <div className="min-h-screen pt-24 px-6 max-w-3xl mx-auto pb-12">
      <Card>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
           ...
           <div>
             <label className="text-sm font-medium text-slate-300 mb-3 block">Product Images (Up to 5)</label>
             <ImageUpload value={images} onChange={setImages} />
           </div>

           <Button type="submit" fullWidth size="lg" loading={loading}>Add Product to Marketplace</Button>
        </form>
      </Card>
    </div>
  );
